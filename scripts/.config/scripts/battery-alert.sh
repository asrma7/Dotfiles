...
if [ "$BATTERY_LEVEL" -gt 99 ] && [ "$BATTERY_DISCHARGING" -eq 0 ] && [ ! -f $FULL_FILE ]; then
	notify-send "Battery Charged" "Battery is fully charged." -i "$iDIR/battery.png" -r 9991
	touch $FULL_FILE
	# If the battery is low and is not charging (and has not shown notification yet)
elif [ "$BATTERY_LEVEL" -le $WARNING_LEVEL ] && [ "$BATTERY_DISCHARGING" -eq 1 ] && [ ! -f $EMPTY_FILE ]; then
	notify-send "Low Battery" "${BATTERY_LEVEL}% of battery remaining." -u critical -i "$iDIR/battery-alert.png" -r 9991
	touch $EMPTY_FILE
	...
	notify-send "Battery Critical" "The computer will shutdown soon." -u critical -i "$iDIR/battery-alert.png" -r 9991
	touch $CRITICAL_FILE
	fi
